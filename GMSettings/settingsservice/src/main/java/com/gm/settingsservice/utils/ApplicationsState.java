...
public class ApplicationsState {
    ...
    /* Anu changes */
    /*
     * public static final AppFilter ON_SD_CARD_FILTER = new AppFilter() { final
     * CanBeOnSdCardChecker mCanBeOnSdCardChecker = new CanBeOnSdCardChecker();
     *
     * public void init() { mCanBeOnSdCardChecker.init(); }
     *
     * @Override public boolean filterApp(ApplicationInfo info) { return
     * mCanBeOnSdCardChecker.check(info); } };
     */
            ...
            for (int i = 0; i < apps.size(); i++) {
                ...
                if (filter == null || filter.filterApp(info)) {
                    synchronized (mEntriesMap) {
                        ...
                        if (DEBUG)
                            Log.i(TAG, "Using " + info.packageName + ": " + entry);
    ...
    public void invalidatePackage(String pkgName) {
        removePackage(pkgName);
        addPackage(pkgName);
    }
    ...
    /* Anu changes */
    /*
     * final class CanBeOnSdCardChecker { final IPackageManager mPm; int
     * mInstallLocation;
     *
     * CanBeOnSdCardChecker() { mPm = IPackageManager.Stub.asInterface(
     * ServiceManager.getService("package")); }
     *
     * void init() { try { mInstallLocation = mPm.getInstallLocation(); } catch
     * (RemoteException e) { Log.e("CanBeOnSdCardChecker",
     * "Is Package Manager running?"); return; } }
     *
     * boolean check(ApplicationInfo info) { boolean canBe = false; if
     * ((info.flags & ApplicationInfo.FLAG_EXTERNAL_STORAGE) != 0) { canBe =
     * true; } else { if ((info.flags & ApplicationInfo.FLAG_SYSTEM) == 0) { if
     * (info.installLocation == PackageInfo.INSTALL_LOCATION_PREFER_EXTERNAL ||
     * info.installLocation == PackageInfo.INSTALL_LOCATION_AUTO) { canBe =
     * true; } else if (info.installLocation ==
     * PackageInfo.INSTALL_LOCATION_UNSPECIFIED) { if (mInstallLocation ==
     * PackageHelper.APP_INSTALL_EXTERNAL) { // For apps with no preference and
     * the default value set // to install on sdcard. canBe = true; } } } }
     * return canBe; } }
     */
}