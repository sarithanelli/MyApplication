 ...
 */
class SettingsVehicleVModeBrakeActivity : BaseActivity() {
    override fun onEventResponse(view: View, obj: Any?) {
        if (obj is Int) {
            dataPoolDataHandler.SETTINGS_MYMODE_BRAKE_POS.set(obj)
            dataPoolDataHandler.SETTINGS_VMODE_MENU [4] = VModeModel(GMSettingsApp.appContext.resources.getString(R.string.drive_mode_brakefeel_response), resources.getStringArray(R.array.my_mode_brake)[obj], Constants.V_MODE_BRAKE)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dataPoolDataHandler.SETTINGS_VMODE_BRAKE.clear()
        GMSettingsApp.appContext.resources.getStringArray(R.array.my_mode_brake).forEach {
            dataPoolDataHandler.SETTINGS_VMODE_BRAKE.add(it)
        }    }
